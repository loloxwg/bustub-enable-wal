...
auto Optimizer::OptimizeSeqScanAsIndexScan(const bustub::AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::SeqScan) {
    ...
    if (cmp_predicate != nullptr && cmp_predicate->comp_type_ == ComparisonType::Equal) {
      ...
      if (dynamic_cast<ConstantValueExpression *>(left_expr.get()) != nullptr &&
        ...
        for (const auto &index : indexs) {
          ...
          if (col_expr->GetColIdx() == schema->GetColIdx(name)) {
            return std::make_shared<IndexScanPlanNode>(seq_scan_plan.output_schema_, seq_scan_plan.table_oid_,
                                                       index->index_oid_, predicate, const_expr);
          }
        }
      }

      if (dynamic_cast<ConstantValueExpression *>(right_expr.get()) != nullptr &&
          dynamic_cast<ColumnValueExpression *>(left_expr.get()) != nullptr) {
