...
void WindowFunctionExecutor::Init() {
  ...
  std::cout << "windouw_function:";
  for (const auto &win_func : plan_->window_functions_) {
    std::cout << fmt::format("type: {}", win_func.second.type_) << std::endl;
  }
  ...
  while (offset_ < pos_.size()) {
    ...
    for (uint32_t i = 0; i < plan_->columns_.size(); i++) {
      auto win_func_ptr = plan_->window_functions_.find(i);
      if (win_func_ptr == plan_->window_functions_.end()) {
        auto col_expr = dynamic_cast<ColumnValueExpression *>(plan_->columns_[i].get());
        values.emplace_back(child_tuple.GetValue(&child_executor_->GetOutputSchema(), col_expr->GetColIdx()));
      } else {
        auto win_func = *win_func_ptr;
        if (window_tables_.find(win_func.first) == window_tables_.end()) {
          window_tables_[win_func.first] = std::make_shared<SimpleWindowHashTable>(win_func.second.type_);
        }
        auto table = window_tables_[win_func.first];
